...
const URL_PARAM_CONFIG = {
  ...
  WORK: { key: 'WORKDuration', type: 'int', min: 1, max: 300 },
...
function loadTimerState() {
  try {
    ...
    if (saved) {
      ...
      timerState = {
        phase: parsed.phase || PHASE.WORK,
        currentSession: currentSession,
        secondsRemaining: parsed.secondsRemaining ?? (settings.WORKDuration * 60),
...
function updateSettingsUI() {
  ...
  if (elements.WORKDuration) elements.WORKDuration.value = settings.WORKDuration;
...
function getNextPhase() {
  if (timerState.phase === PHASE.WORK) {
    ...
    return { phase: PHASE.WORK, incrementSession: true };
  }

  return { phase: PHASE.WORK, incrementSession: false };
}

function advancePhase(fromTick = false) {
  const { phase: nextPhase, incrementSession } = getNextPhase();
  const workAlarm = new Audio('はしっこポケット.mp3');
  const breakAlarm = new Audio('jingle26.mp3');
...
function resetTimer() {
  ...
  timerState.secondsRemaining = settings.WORKDuration * 60;
  timerState.isRunning = false;
  timerState.lastTick = null;
...
function handleSettingChange(key, value) {
  ...
  if (!timerState.isRunning && timerState.phase === PHASE.WORK && timerState.currentSession === 1) {
    if (key === 'WORKDuration') {
...
function setupEventListeners() {
  ...
  if (elements.WORKDuration) {
    elements.WORKDuration.addEventListener('change', (e) => {
      handleSettingChange('WORKDuration', e.target.value);
    });
  }
...
function parseUrlParams() {
  ...
  if (hasOverrides) {
    const settingsChanged =
      ...
      oldSettings.WORKDuration !== settings.WORKDuration ||
...
function generateObsUrl() {
  ...
  params.set('WORK', settings.WORKDuration);
